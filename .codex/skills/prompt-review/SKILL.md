...
description: >
  ...
  と依頼したとき、または `$prompt-review` と明示したときに使用する。
  過去のAIエージェント対話履歴（Codex, Claude Code, GitHub Copilot Chat, Cline, Roo Code,
  Windsurf, Antigravity）を読み取り、技術理解度・プロンプティングパターン・AI依存度を推定して
  レポートを生成する。
---
...
Antigravity のログを自動検出し、フィルタ済みのJSON を標準出力に返す。
...
```json
{
  "summary": {
    "total_messages": 2616,
    "detected_tools": ["Codex", "Claude Code", "GitHub Copilot Chat"],
    "filter_days": null,
    "filter_project": null
  },
  "sources": [
    {
      ...
      "status": "検出",
      "messages": [
        {"text": "プロンプト本文", "timestamp": "2026-03-12 09:44", "project": "timekeeper"}
      ],
      "period": "2026-03-10 08:37 〜 2026-03-12 09:44"
    }
  ],
  "project_stats": {
    "timekeeper": {"count": 668, "tools": ["Codex", "Claude Code"]},
...
Codex や Claude Code はユーザーに「〜しますか？」と確認を求めることが多く、それに対するユーザーの短い肯定応答はプロンプティング能力や技術理解度の分析に価値がない。以下のようなメッセージは分析対象から**除外**すること:
...
1. `reports/` ディレクトリが存在しない場合は `mkdir -p reports` を実行する
2. `reports/prompt-review-YYYY-MM-DD.md` にレポートを書き出す
...
- どのツールからの情報かを括弧書きで明記する（例:「(Codex)」「(Claude Code)」「(Copilot Chat)」）
