...
def normalize_epoch_ms(ts) -> int | None:
    """秒/ミリ秒どちらのepochでもミリ秒に正規化する"""
    if ts is None:
        return None
    try:
        value = int(ts)
...
def matches_project_filter(project_filter: str | None, *values: str) -> bool:
    """部分一致のプロジェクトフィルタを判定する"""
...
def extract_codex_user_text(content) -> str:
    """Codexのmessage.contentからユーザーテキストを抽出する"""
    if isinstance(content, str):
        return sanitize_text(content.strip())
...
def collect_codex(cutoff_ms: int | None, project_filter: str | None) -> dict:
    """Codex の history.jsonl および sessions JSONL からユーザープロンプトを収集"""
    result = {"tool": "Codex", "status": "未検出", "messages": [], "period": ""}
    ...
    if sessions_dir.exists():
        session_files = sorted(
            [f for f in sessions_dir.rglob("*.jsonl") if f.is_file()],
                ...
                with open(session_file, "r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        if entry_type == "session_meta" and isinstance(payload, dict):
                            current_session_id = payload.get("id", current_session_id)
                            current_cwd = payload.get("cwd", "") or current_cwd
                            session_context[current_session_id] = {
                                "cwd": current_cwd,
                                "project": Path(current_cwd).name if current_cwd else "unknown",
                            }
                            continue
                        ...
                        text = extract_codex_user_text(payload.get("content", ""))
                        if not text:
                            continue
                        if any(text.startswith(p) for p in skip_patterns):
                            continue

                        ts_ms = iso_to_ms(entry.get("timestamp", ""))
                        if cutoff_ms and ts_ms and ts_ms < cutoff_ms:
                            continue

                        ctx = session_context.get(current_session_id, {})
                        cwd = current_cwd or ctx.get("cwd", "")
                        project = Path(cwd).name if cwd else ctx.get("project", "unknown")
                        if not matches_project_filter(project_filter, project, cwd, session_file.as_posix()):
                            continue

                        dedup_key = f"{current_session_id}:{ts_ms}:{text[:100]}"
        ...
        try:
            with open(history_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    text = sanitize_text(entry.get("text", "").strip())
                    if not text:
                        continue
                    if any(text.startswith(p) for p in skip_patterns):
                        continue

                    ts_ms = normalize_epoch_ms(entry.get("ts"))
                    if cutoff_ms and ts_ms and ts_ms < cutoff_ms:
                        continue
                    ...
                    dedup_key = f"{session_id}:{ts_ms}:{text[:100]}"
                    if dedup_key in seen_texts:
                        continue
                    seen_texts.add(dedup_key)
    ...
    if messages:
        result["status"] = "検出"
        result["messages"] = sorted(messages, key=lambda m: m["timestamp_ms"])
...
def collect_claude_code(cutoff_ms: int | None, project_filter: str | None) -> dict:
    ...
    if history_path.exists():
        with open(history_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if not matches_project_filter(project_filter, project_name, project):
                    continue
    ...
    if projects_dir.exists():
        for project_dir in projects_dir.iterdir():
            ...
            if not matches_project_filter(project_filter, project_name_from_dir, dir_name):
                continue
